...
LEFT JOIN (SELECT {id}, ind_hard, ind_soft FROM od_closest WHERE dest = 8) any_pt 
      ON parcel_dwellings.{id} = any_pt.{id};
'''.format(id = points_id)

create_ind_walkability = '''
DROP TABLE IF EXISTS ind_walkability;
CREATE TABLE ind_walkability AS
SELECT dl.{id}, z_dl_hard, z_dl_soft, z_sc, z_dd, z_dl + z_sc + z_dd AS wa_hard, z_dl_soft + z_sc + z_dd AS wa_soft,
FROM (SELECT {id},(dl_hard - AVG(dl_hard) OVER())/stddev_pop(dl_hard) OVER() as z_dl_hard,
     (SELECT {id},(dl_soft - AVG(dl_soft) OVER())/stddev_pop(dl_soft) OVER() as z_dl_soft FROM ind_daily_living) dl
LEFT JOIN
