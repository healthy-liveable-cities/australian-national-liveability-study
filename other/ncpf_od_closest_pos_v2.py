...
if __name__ == '__main__':
    exit_message = '''
        Please specify a locale and the type of POS analysis you are calculating. 
        
        The required format is:
        python script [locale] [analysis] 
        
        Examples of code to run expressed in this format are:
        python 15b_od_aos_testing_melb_vpa.py melb any
        python 15b_od_aos_testing_melb_vpa.py melb large
        
        '''  
    if len(sys.argv) < 3:
        sys.exit(exit_message)
    elif sys.argv[2] not in ['any','large']:
        sys.exit(exit_message)
...
analysis_dict = {"any":"any POS in distance <= 400 m",
                 "large":"POS > 1.5 Ha  in distance <= 400 m"}

aos_public_ha_min = {"any":0,
                     "large":1.5}                 
                 
this_analysis = analysis_dict[ind]      
this_ind = ind

os_source = "aos_public_osm"
...
aos_points = "aos_nodes_30m_pos{pos_suffix}".format(pos_suffix = pos_suffix)   
  ...
  SELECT 1 
   WHERE EXISTS (SELECT 1 
                  ...
                  WHERE h.orig_fid = {hex} 
                    AND aos_ha_public > {aos_public_ha_min}
                    AND ST_Intersects(h.geom,p.geom) 
               GROUP BY h.orig_fid);
  '''.format(region = region,hex = hex[0],os_source = os_source,aos_public_ha_min=aos_public_ha_min[this_ind])
  ...
  if len(buffer_intersects) == 0:
    ...
    if len(hex_intersects) > 0:
      ...
      '''.format(id = points_id.lower(),
                 ...
                 hex = hex[0],
                 os_source = os_source
                 ,aos_public_ha_min=aos_public_ha_min[this_ind])
      ...
      curs.execute("DROP TABLE IF EXISTS aos_temp_hex_{hex}".format(hex=hex[0]))
...
if __name__ == '__main__':
  ...
  task = 'POS in 400m OD analysis for NCPF'
