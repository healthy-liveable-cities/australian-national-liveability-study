...
if locale!='australia':
    ...
    os_public_02_hard                            ,
    trans_06_hard
    FROM li_inds_sa2_dwelling
    ORDER BY study_region, sa2_name_2016 ASC;    
    '''.format(locale)
else:
    sql = '''
    SELECT
    ...
    a.os_public_02_hard                            ,
    a.trans_06_hard
    FROM li_inds_sa2_dwelling a
    LEFT JOIN wa_1600m_ntnl_sa2 w USING (sa2_name_2016)
...
df.to_csv('D:/ntnl_li_2018_template/data/study_region/dh_inds/dh_inds_{}_sa2_dwellings_2018_{}.csv'.format(locale,time.strftime('%Y%m%d')),index=False)
...
# if locale!='australia':
    ...
    # command = 'pg_dump -U {db_user} -h localhost -Fc -t "parcel_indicators" -t "dest_closest_indicators" -t "dest_array_indicators" -t "od_aos_jsonb" -t "open_space_areas" -t "ind_summary" -t "exclusion_summary" -t "area_indicators_mb_json" -t "area_linkage" -t "li_inds_sa2_dwelling" {db} > {out_file}'.format(db = db,db_user = db_user,out_file=out_file)    
