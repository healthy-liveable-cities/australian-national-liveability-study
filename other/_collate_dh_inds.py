...
out_file = os.path.join(dh_dir,'dh_inds_sa1_{}.csv'.format(time.strftime('%Y%d%m')))
dh_inds_csv = []
for root, dirs, files in os.walk('D:/ntnl_li_2018_template/data/study_region/dh_inds'):
  for file in files:
    if file.endswith('.csv') and not file.startswith('dh_inds_sa1_'):
        dh_inds_csv.append(os.path.join(root,file))
...
df.sort_values('sa1_maincode_2016', inplace=True)
