...
def create_service_areas(hex):
    ...
    try:
        session = get_session()
        pid = str(session)[-17:-1]
        temp_gdb = os.path.join(temp,f"{locale}-{pid}.gdb").lower().replace('_','-')
        ...
        if not os.path.exists(temp_gdb):
            ...
            outSAResultObject = arcpy.na.MakeServiceAreaAnalysisLayer(
                                          ...
                                          output_type="LINES", # equivalent of 50m network buffer
        ...
        # engine.dispose()
        # # text = f"#{n}, est. {total:<04.2}s"
        # # for _ in trange(total, desc=text):
            # # sleep(interval)
        # # if n == 6:
            # # tqdm.write("n == 6 completed.")
            # # tqdm.write("`tqdm.write()` is thread-safe in py3!")
        # tqdm.write(f"{pid}: {hex}")
    except:
        raise
    finally:
...
if __name__ == '__main__':
    try:
        ...
        with ThreadPoolExecutor(max_workers=nWorkers) as p:
            r = list(tqdm(p.map(create_service_areas, iteration_list), total=len(iteration_list), unit='polygon'))
        ...
        print("Please manually check all non-required temp files are deleted after running.")
        for gdb in glob.glob(f"{temp}/li*"):
            try:
                os.remove(gdb)
            except: 
                print(f"  - {gdb}")
