...
result_table = "od_distances_3200m_20191129"
...
def ODMatrixWorkerFunction(hex): 
  ...
  try:   
    ...
    remaining_dest_list = [x for x in hex_dests if x[0] in destination_list and x[0] not in completed_dest]
    place = "before not_processed_dest"
    # not_processed_dest = [(x[0],0) for x in destination_list if x[0] not in [d[0] for d in hex_dests]]
    not_processed_dest = [(x,0) for x in destination_list if x not in [d[0] for d in hex_dests]]
...
if __name__ == '__main__':
  ...
  evaluate_progress = '''
   ...
   FROM (SELECT COUNT(DISTINCT(dest_class)) FROM dest_type 
               ...
               and count > 0
               and dest_class IN ({dest_list})) destinations,
        (SELECT COUNT(*) FROM parcel_dwellings) parcels,
        (SELECT processed FROM {progress}) processed;
