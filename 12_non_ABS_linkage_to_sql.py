# Script:  20a_non_ABS_linkage_to_SQL.py
# Purpose: Creates table with non-ABS area linkage codes for parcel
# Author:  Julianna Rozek and Carl Higgs
# Date:    20/05/2017
...
## INPUT SETTINGS
# define spatiotemporal parameters for study region naming convention
locale = parser.get('data', 'locale')
region = parser.get('data', 'region')
year   = parser.get('data', 'target_year')
study_region = '{0}_{1}_{2}'.format(locale,region,year)
buffer = parser.get('data', 'study_buffer')
units = parser.get('data', 'units')
buffered_study_region = '{0}_{1}{2}'.format(study_region,buffer,units)
db          = 'li_{0}_{1}'.format(locale,year).lower()
folderPath    = parser.get('data', 'folderPath')
gdb    = os.path.join(folderPath,'{}.gdb'.format(db))

# ArcGIS environment settings
arcpy.env.workspace = gdb
arcpy.env.overwriteOutput = True 
sde_connection = os.path.join(folderPath,'{}.sde'.format(db))
spatial_reference = arcpy.SpatialReference(parser.get('data', 'SpatialRef'))
