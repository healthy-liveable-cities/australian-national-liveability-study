...
ind_avg = ',\n'.join(ind_matrix.apply(lambda x: 'AVG({mult}*"{i}") AS "{i}"'.format(i=x.name,mult='100.0' if x['agg_scale']==100 else '1.0'),axis=1))
ind_sd = ',\n'.join(ind_matrix.apply(lambda x: 'stddev_samp({mult}*"{i}") AS "{i}"'.format(i=x.name,mult='100.0' if x['agg_scale']==100 else '1.0'),axis=1))

# Create query for indicator range (including scaling of percent variables)
ind_range = ',\n'.join(ind_matrix.apply(lambda x: 'ROUND(MIN({mult}*"{ind}")::numeric,1)::text || $$ to $$ ||ROUND(MAX({mult}* "{ind}")::numeric,1)::text AS "{ind}"'.format(ind=x.name,mult = '100.0' if x.agg_scale == 100 else '1.0'),axis=1))

# Create query for median       
ind_median = ',\n'.join(ind_matrix.apply(lambda x: 'round(percentile_cont(0.5) WITHIN GROUP (ORDER BY {mult}*"{ind}" )::numeric,1) AS "{ind}"'.format(ind=x.name,mult = '100.0' if x.agg_scale == 100 else '1.0'),axis=1))
...
ind_iqr = ',\n'.join(ind_matrix.apply(lambda x: 'round(percentile_cont(0.25) WITHIN GROUP (ORDER BY {mult}*"{ind}" )::numeric,1)::text || $$ to $$ || round(percentile_cont(0.75) WITHIN GROUP (ORDER BY {mult}*"{ind}" )::numeric,1)::text AS "{ind}"'.format(ind=x.name,mult = '100.0' if x.agg_scale == 100 else '1.0'),axis=1))

# Create query for percentile           
ind_percentile = ',\n'.join(ind_matrix.apply(lambda x: 'round(100*cume_dist() OVER(ORDER BY "{ind}" {polarity})::numeric,0) AS "{ind}"'.format(ind=x.name,polarity = x.polarity),axis=1))

# Map indicator queries
map_ind_raw = ',\n'.join(ind_matrix.apply(lambda x: 'round(raw."{ind}"::numeric,1) AS "r_{ind}"'.format(ind=x.name),axis=1))                           
map_ind_sd = ',\n'.join(ind_matrix.apply(lambda x: 'round(sd."{ind}"::numeric,1) AS "sd_{ind}"'.format(ind=x.name),axis=1))
map_ind_percentile = ',\n'.join(ind_matrix.apply(lambda x: 'round(perc."{ind}"::numeric,1) AS "p_{ind}"'.format(ind=x.name),axis=1))    
map_ind_range = ',\n'.join(ind_matrix.apply(lambda x: 'range."{ind}" AS "d_{ind}"'.format(ind=x.name),axis=1))  
map_ind_median = ',\n'.join(ind_matrix.apply(lambda x: 'median."{ind}" AS "med_{ind}"'.format(ind=x.name),axis=1))  
map_ind_iqr = ',\n'.join(ind_matrix.apply(lambda x: 'iqr."{ind}" AS "m_{ind}"'.format(ind=x.name),axis=1))  
