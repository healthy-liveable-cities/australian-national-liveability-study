# Script:  12_od_pt_list_analysis.py
...
pt_source = gtfs_all_stops_headway
if pt_source in ['','NULL','NaN']:
    sys.exit('Public transport for this analysis has not been defined; see the config parameter "gtfs_all_stops_headway"')
pt_points   =  'gtfs_all_stops_headway'
pt_pointsID =  'fid'

print("\nCopy detailed PT data to postgis..."),
urban_region = gpd.GeoDataFrame.from_postgis('''SELECT geom FROM buffered_study_region''',engine)
command = (
...
arcpy.env.overwriteOutput = True 
arcpy.CopyFeatures_management('public.gtfs_all_stops_headway', os.path.join(gdb_path,'gtfs_all_stops_headway')) 
print("Done.")
...
if pid !='MainProcess':
  ...
  arcpy.MakeFeatureLayer_management(pt_points, "pt_pointsLayer")   
...
# by minimum distance to an entry mode
recInsert      = '''
  INSERT INTO {table} ({id}, pt_id, mode, distance)  
  SELECT DISTINCT ON (gnaf_pid, pt_id) gnaf_pid, pt_id, mode, distance
   FROM  
   (VALUES 
...
# mode is retained for verification purposes; 
...
# required distance
recUpdate      = '''
  ) v({id}, pt_id, mode, distance) 
  ORDER BY gnaf_pid, pt_id, distance ASC 
  ON CONFLICT ({id}, pt_id) 
    DO UPDATE
...
def ODMatrixWorkerFunction(hex): 
  ...
  try:
    ...
    # print(place)   
    B_selection = arcpy.SelectLayerByLocation_management('pt_pointsLayer', 'WITHIN_A_DISTANCE', hex_selection, '3200 Meters')
    ...
    if B_pointCount > 0:  
      ...
      if len(hex_intersects) > 0:
        evaluate_os_intersection = '''
        INSERT INTO od_aos ({id}, pt_id,distance)
        SELECT {id}, 
                pt_id,
                0
      ...
      for chunk in chunks(to_do_points,sqlChunkify):
        ...
        arcpy.AddLocations_na(in_network_analysis_layer = outNALayer, 
          ...
          field_mappings                 = "Name {} #".format(pt_pointsID), 
          search_tolerance               = "{} Meters".format(tolerance), 
          search_criteria                = "{} SHAPE;{} NONE".format(network_edges,network_junctions), 
          append                         = "CLEAR", 
          snap_to_position_along_network = "NO_SNAP", 
        ...
        if result[1] == u'true':
          ...
          for outputLine in outputLines:
            ...
            pt_pair = od_pair[1].split(',')
            aos = int(pt_pair[0])
            mode = int(pt_pair[1])
            distance = int(round(outputLine[1]))
            place = "before chunk append, gnaf = {}".format(pid)
            chunkedLines.append('''('{pid}',{aos},{mode},{distance})'''.format(pid = pid,
                                                                               aos = aos,
                                                                               mode = mode,
                                                                               distance  = distance))
...
if __name__ == '__main__':
  ...
  createTable     = '''
  -- DROP TABLE IF EXISTS od_pt_jsonb;
  CREATE TABLE IF NOT EXISTS od_pt_jsonb
  (
  ...
  od_pt_progress_table = '''
    DROP TABLE IF EXISTS od_pt_progress;
    CREATE TABLE IF NOT EXISTS od_pt_progress 
       (processed int);
    '''
  curs.execute(od_pt_progress_table)
  conn.commit()
  print("Done.")
  
  
  print("Divide work by hexes for multiprocessing, only for parcels not already processed... "),
  # evaluated against od_pt_jsonb as the id field has no duplicates in this table
  antijoin = '''
    SELECT p.hex_id, 
           jsonb_agg(jsonb_strip_nulls(to_jsonb(p.{id}))) AS incomplete
    FROM parcel_dwellings p
    WHERE NOT EXISTS 
    (SELECT 1 FROM od_pt_jsonb s WHERE s.{id} = p.{id})
    GROUP BY p.hex_id;
  '''.format(id = points_id.lower())
  ...
  curs.execute('''INSERT INTO od_pt_progress (processed) VALUES ({})'''.format(processed))
  conn.commit()
  print("Done.")
  ...
  curs.execute('''CREATE INDEX IF NOT EXISTS idx_od_pt_jsonb ON od_pt_jsonb ({id});'''.format(id = points_id.lower()))
  curs.execute('''CREATE INDEX IF NOT EXISTS idx_od_pt_jsonb_pt_id ON od_pt_jsonb ((attributes->'pt_id'));''')
  curs.execute('''CREATE INDEX IF NOT EXISTS idx_od_pt_jsonb_distance ON od_pt_jsonb ((attributes->'distance'));''')
  conn.commit()
